Assets = {
	...
	--Doydoy image
	Asset("ATLAS", "images/doydoybaby.xml"),
	Asset("ATLAS", "images/doydoyteen.xml"),
...
local SHOWDOYDOYS = GetModConfigData("SHOWDOYDOYS") and not DST

local DOYDOY_COUNTER = 0

local function Doydoycount(inst)
...
local function CountPrefab(prefab)
    local count = 0
...
AddPrefabPostInit("doydoy", Doydoycount)
AddPrefabPostInit("doydoybaby", Doydoycount)
...
local function AddSeasonBadge(self)
	...
	if COMPACTSEASONS then
		...
		temp_nudge = temp_nudge + (SHOWDOYDOYS and 1 or 0)
...
local function StatusPostConstruct(self)
	...
	if SHOWDOYDOYS then	
		self.doydoys = self:AddChild(Minibadge("doydoys", self.owner))
		...
		if SHOWTEMPBADGES then
			...
			self.doydoybadge:SetPosition(41, nudge-35.5)
			--self.doydoybadge.head:GetAnimState():SetBank('doydoy')
			--self.doydoybadge.head:GetAnimState():SetBuild('krampus_build')
			--self.doydoybadge.head:GetAnimState():SetPercent('hit', 1)
			self.doydoybadge.head:SetScale(0.5)
			self.doydoybadge.head:SetPosition(0, -5)
			self.doydoys.bg:SetPosition(4, nudge-46)
			self.doydoys.num:SetPosition(10, nudge-46.5)
			self.doydoys.num:SetScale(0.9, .7, 1)
		end
		nudge = nudge - 30
	end
